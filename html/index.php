					...
					<div class="card-body">
						...
						<p class="card-text">
							<details open>
								...
								<ul>
									<?php
									$res = $con->simple_exec_obj("SELECT * FROM forums WHERE category = 'programming'");
							...
							<details open>
								<summary>Cafecoder</summary>
								<ul>
									<?php
									$res = $con->simple_exec_obj("SELECT * FROM forums WHERE category = 'cafecoder'");
									foreach ($res as $values) {
										echo '<a href="forums.php?forum_id=' . $values["id"] . '">' . $values["name"] . '</a><br>' . "\n";
									}
									?>
